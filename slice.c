...
next_file_name (void)
{
  ...
  strftime(buffer,60,"slice-%Y%m%d_%T.",localtime(&curtime));
  sprintf(timestamp,"%s%ld.raw",buffer,tv.tv_usec);
  fprintf(stdout,_("%s%ld.raw\n"),buffer,tv.tv_usec);
...
*/
      ...
      size_t suffix_length = strlen (timestamp);
...
*/ 
      memcpy (outfile_mid, timestamp, suffix_length);
